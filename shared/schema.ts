...
// --- NEW: tbs.bank_accounts ---
export const bankAccounts = pgTable("bank_accounts", {
  id: uuid("id").defaultRandom().primaryKey(), // maps to gen_random_uuid()
...
// --- NEW: tbs.terms (singleton row with id='singleton') ---
...
// --- NEW: tbs.sequences (for "Quotation X" / "Invoice X") ---
export const sequences = pgTable("sequences", {
  id: text("id").primaryKey(),              // 'quotation' | 'invoice'
...
export const accessPasswords = pgTable("access_passwords", {
  area: varchar("area", { length: 50 }).primaryKey(), // 'pricing' | 'quotations'
...
export const insertAccessPasswordSchema = z.object({
  area: z.enum(["pricing", "quotations"]),
...
export const selectTermsSchema = createSelectSchema(terms);
...
export const insertTermsSchema = createInsertSchema(terms).omit({
  ...
  // if you ever allow setting a custom id (usually not), keep it optional:
  id: z.literal("singleton").optional(),
});
...
export type Terms = z.infer<typeof selectTermsSchema>;
...
// --- NEW: Sequence type schema (for routes)
export const sequenceTypeSchema = z.enum(["quotation", "invoice"]);
